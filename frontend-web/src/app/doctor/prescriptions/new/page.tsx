...
import { Pill, Save, ShieldCheck, ArrowLeft, User, Printer, Trash2, Plus, PlusCircle, CheckCircle2, Search, Activity, Stethoscope } from "lucide-react"
...
export default function NewPrescriptionPage() {
  ...
  useEffect(() => {
    ...
    const pts = users.filter((u: any) => u.role === "patient")
    setPatients(pts)
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!doctor) return alert("Login required")
    
    // 1. Create Prescription
    const rxId = "rx_" + generateId()
    const opId = "op_" + generateId()
    ...
    // 2. Auto Pharmacy Order
    const phOrders = getData("pharmacyOrders") || []
    phOrders.unshift({
      id: "pho_" + generateId(),
      prescriptionId: rxId,
      patientId: selectedPatientId,
    ...
    saveData("pharmacyOrders", phOrders)

    // 3. Auto Lab Order if "test" or "lab" mentioned
    if (advice.toLowerCase().includes("test") || advice.toLowerCase().includes("lab")) {
       ...
       labOrders.unshift({
          ...
          patientId: selectedPatientId,
          doctorId: doctor.id,
          testName: advice || "Basic Panel",
          status: "pending",
          createdAt: new Date().toISOString()
       })
       saveData("labOrders", labOrders)
    }
      ...
      <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-8 pb-32 tracking-tight">
        <div className="flex items-center justify-between">
           <div className="flex items-center gap-4">
              ...
              <div>
                 <h1 className="text-2xl font-black italic tracking-tighter uppercase">{templateType ? `${templateType}` : "Digital"} Prescription</h1>
           ...
           <Button variant="ghost" className="h-12 px-6 rounded-2xl text-[10px] uppercase font-black tracking-widest text-medical-blue hover:bg-medical-blue/5">
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
           ...
           <div className="bg-slate-900 rounded-[2.5rem] p-8 border border-slate-800 shadow-2xl space-y-8 text-white">
              <div className="flex items-center justify-between">
                 <h3 className="font-extrabold text-sm uppercase tracking-widest flex items-center gap-2 opacity-60 italic">
                    <Pill className="w-5 h-5 text-medical-pink" /> Medications / Rx
                 </h3>
              ...
              <div className="space-y-6">
                 {medications.map((med, idx) => (
                    <motion.div key={med.id} initial={{ x: -10, opacity: 0 }} animate={{ x: 0, opacity: 1 }} className="grid grid-cols-1 md:grid-cols-12 gap-4 items-end pb-8 border-b border-white/5">
                       <div className="md:col-span-1 flex flex-col items-center justify-center pt-8">
                       ...
                       <div className="md:col-span-4 space-y-2">
                          ...
                          <Input required value={med.name} onChange={(e) => {
                             const newList = [...medications]
                             newList[idx].name = e.target.value
                             setMedications(newList)
                          }} className="rounded-2xl h-14 bg-white/10 border-white/5 focus:bg-white/20 transition-all font-bold px-5 text-white placeholder:text-white/20" placeholder="e.g. Amlodipine 5mg" />
                       </div>
                       <div className="md:col-span-2 space-y-2">
                          <label className="text-[10px] font-black uppercase text-white/30 ml-1">Dosage</label>
                          <Input required value={med.dosage} onChange={(e) => {
                             const newList = [...medications]
                             newList[idx].dosage = e.target.value
                             setMedications(newList)
                          }} className="rounded-2xl h-14 bg-white/10 border-white/5 focus:bg-white/20 transition-all font-bold px-5 text-white" placeholder="1 Tab" />
                       </div>
                       <div className="md:col-span-2 space-y-2">
                          <label className="text-[10px] font-black uppercase text-white/30 ml-1">Timing</label>
                          <select value={med.timing} className="w-full h-14 rounded-2xl bg-white/10 border-white/5 focus:bg-white/20 transition-all font-bold px-5 outline-none appearance-none cursor-pointer">
                       ...
                       <div className="md:col-span-2 space-y-2">
                          ...
                          <Input value={med.duration} className="rounded-2xl h-14 bg-white/10 border-white/5 focus:bg-white/20 transition-all font-bold px-5 text-white" placeholder="5 Days" />
                       </div>
                       <div className="md:col-span-1 pb-1">
                          {medications.length > 1 && (
                            <button type="button" onClick={() => removeMedication(med.id)} className="w-12 h-14 bg-red-500/10 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl flex items-center justify-center transition-all opacity-40 hover:opacity-100">
           ...
           <div className="flex flex-col md:flex-row gap-8">
              <div className="flex-1 bg-white rounded-[2rem] p-8 border border-medical-grey/60 space-y-6">
                 <h3 className="font-extrabold text-sm uppercase tracking-widest flex items-center gap-2">
                    <Stethoscope className="w-5 h-5 text-medical-green" /> Advice / Lab Tests
                 </h3>
                 <textarea value={advice} onChange={(e) => setAdvice(e.target.value)} className="w-full h-40 rounded-2xl bg-medical-grey/20 border-medical-grey/40 focus:bg-white transition-all font-medium p-6 outline-none resize-none" placeholder="Enter special advice, precautions or suggested lab tests (e.g. CBC test)..."></textarea>
              </div>
              <div className="w-full md:w-80 flex flex-col gap-4 justify-end">
                 <Button type="submit" className="h-16 rounded-[1.5rem] bg-medical-green hover:bg-medical-green/90 text-white font-black text-xs uppercase tracking-[0.3em] shadow-2xl shadow-medical-green/20">

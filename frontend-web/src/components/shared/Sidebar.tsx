...
import {
  Home, Rss, Video, History, Users, Building2, Microscope,
  Droplet, Ambulance, Pill, CalendarCheck, MessageSquare, User,
  Settings, HeartPulse
} from "lucide-react"
...
// Role labels shown in sidebar bottom profile
...
export default function Sidebar() {
  ...
  const menuItems =
    role === "doctor"   ? doctorItems :
    role === "admin"    ? adminItems  :
    role === "hospital" ? hospitalItems :
                          userItems
    ...
    <aside className="w-64 bg-white border-r h-screen sticky top-0 hidden md:flex flex-col flex-shrink-0 z-50">
      ...
      <div className="p-4 border-t border-medical-grey mt-auto">
        <div className="bg-medical-grey/30 rounded-2xl p-3 flex items-center gap-3">
          <div className="w-10 h-10 rounded-full bg-medical-green flex items-center justify-center text-white font-black text-xs overflow-hidden flex-shrink-0">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[10px] font-black uppercase truncate">{currentUser?.name || "User"}</p>
            <p className="text-[8px] font-bold text-muted-foreground uppercase">{roleLabel[role] || "User"}</p>
          </div>
        </div>
      </div>
      ...
      <style jsx global>{`
        ...
        .custom-scrollbar:hover::-webkit-scrollbar-thumb { background: #34c759; }
      `}</style>
    </aside>
  )
}
